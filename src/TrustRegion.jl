...
mutable struct TrustRegionMethod <: AbstractMethod
    ...
    all_fx0::Vector{Float64}	# all best objective values
...
function termination_test(method::TrustRegionMethod)::Bool
    ...
    push!(method.all_fx0, method.fx0[1])
    ...
    #     println("TERMINATION: Optimal ", sum(method.fx0) - sum(method.θ))
    #     @show sum(method.fx0), sum(method.θ)
    #     return true
    # end
