...
function solve_model!(model::BundleModel)
    ...
    JuMP.optimize!(m)
    if JuMP.termination_status(m) ∉ [MOI.OPTIMAL, MOI.ALMOST_OPTIMAL, MOI.LOCALLY_SOLVED]
        set_optimizer_attribute(m, "max_iter", 300000)
        JuMP.optimize!(m)
        if JuMP.termination_status(m) ∉ [MOI.OPTIMAL, MOI.ALMOST_OPTIMAL, MOI.LOCALLY_SOLVED]
            set_optimizer_attribute(m, "max_iter", 300000)
            set_optimizer_attribute(m, "tol", 1e-6)
            set_optimizer_attribute(m, "acceptable_tol", 1e-4)
            JuMP.optimize!(m)
        end
    end
